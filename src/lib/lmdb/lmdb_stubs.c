...
CAMLprim value ocaml_lmdb_sync(value _env)
{
  CAMLparam1(_env);
...
CAMLprim value ocaml_lmdb_open(value file, value _nosync)
{
  CAMLparam2(file, _nosync);
  CAMLlocal1(wrapper);
  uint64_t n = 1024;
  size_t map_size = n * n * n * n;
  int flags = MDB_NOTLS | MDB_NOSUBDIR;
  MDB_env *env = NULL;
  int rc;
  ...
  rc = mdb_env_open(env, String_val(file), flags, 0755);
  if (rc != 0) {
    mdb_env_close(env);
    raise_lmdb_error(rc);
  }
...
CAMLprim value ocaml_lmdb_put(value _env, value _hashes)
{
